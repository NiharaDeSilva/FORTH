...
main = hspec $ do
  ...
  describe "interpret" $ do
    ...
    context "Stack not empty warning" $ do
        it "leaves stack non-empty and prints a warning" $ do
            let (stack, output) = interpret "5 10"
            stack `shouldBe` [Integer 10, Integer 5] -- Expect stack to hold values
            output `shouldBe` "" -- No print output 